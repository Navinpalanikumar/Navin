...
// Typing animation on hero subtitle instead — preserves .highlight span's color animation
window.addEventListener('load', () => {
    const subtitle = document.querySelector('.hero-subtitle');
    if (subtitle) {
        const originalText = subtitle.textContent.trim();
        typeWriter(subtitle, originalText, 55);
    }
});

// Experience "show more / show less" toggle
function toggleExp(listId, btn) {
    ...
    btn.innerHTML = isExpanded
        ? `<i class="fas fa-chevron-down"></i> Show ${count} more`
